...
export default function DriverHistoryPage() {
  ...
  return (
    <Layout>
      <div className="mb-6">
        <h2 className="text-3xl font-display font-bold text-primary">{t("driverHistory.title")}</h2>
        <p className="text-muted-foreground">{t("driverHistory.subtitle")}</p>
      </div>
      <div className="bg-card rounded-xl border border-border">
        {isLoading ? (
          <div className="p-6 text-muted-foreground">{t("driverHistory.loading")}</div>
        ) : rows.length === 0 ? (
          <div className="p-6 text-muted-foreground">{t("driverHistory.empty")}</div>
        ) : (
              ...
              <div key={s.id} className="p-4 flex items-start justify-between">
                <div className="flex-1 pr-4">
                  <div className="text-sm text-muted-foreground">
                    {format(new Date(s.dateTime), "dd/MM/yyyy HH:mm", { locale: dateLocale })}
                  </div>
